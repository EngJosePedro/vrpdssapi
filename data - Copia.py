...
if __name__ == '__main__':
    ...
    try:
        ...
        ArcsEstudados = [(i,j) for i in range(len(latlngs)) for j in range(len(latlngs)) if i!=j]
        ArcsEstudados = sample(ArcsEstudados, len(ArcsEstudados))
        
        # add alguns arcos
        
        #ArcsEstudados = [(i,j) for j in range(len(latlngs)) for i in range(0, int(len(latlngs)/5))]
        print("start")
        while len(ArcsEstudados):
            ref, ArcsEstudados = getRef(latlngs, ArcsEstudados.copy(), ref = [], n=70)
            
            if len(ref)==0: continue
            #print(len(ArcsEstudados), ref)
            ...
            if len(input_data)>=nP:
                ...
                if 0 in unique:
                    print(len(ArcsEstudados), len(latlngs) * len(latlngs) - len(latlngs) , teste[0] , len(latlngs)*len(latlngs) - teste[0] - len(latlngs),time.time() - t)
...
"""dado_Final = {"d_ij": {}, "t_ij": {}}

for i, node1 in enumerate(nodes):
    name1 = node1["name"]
    latlng1 = (node1["latlng"]["lat"], node1["latlng"]["lng"])
    for j, node2 in enumerate(nodes):
        name2 = node2["name"]
        latlng2 = (node2["latlng"]["lat"], node2["latlng"]["lng"])"""
